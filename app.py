...
def criar_scraper_avancado():
    ...
    # Criar scraper com configurações avançadas
    scraper = cloudscraper.create_scraper(
        ...
        delay=random.uniform(3, 6)  # Delay aleatório entre 3-6 segundos
    )
...
def buscar_dados_fbref(url_base, nome_time, competicao):
    ...
    try:
        ...
        # Delay inicial mais longo para parecer mais humano
        ...
        if '403' in erro_msg or 'Forbidden' in erro_msg:
            return None, (
                "⚠️ O FBref bloqueou temporariamente o acesso.\n\n"
                "💡 Sugestões:\n"
                "1. Aguarde 1-2 minutos e tente novamente\n"
                "2. O site pode estar com proteção extra no momento\n"
                "3. Tente com outro time primeiro"
            )
        elif '429' in erro_msg or 'Too Many' in erro_msg:
            return None, (
                "⚠️ Muitas requisições em pouco tempo.\n\n"
...
st.info("ℹ️ **Nota:** O FBref tem proteção anti-bot. Se receber erro 403, aguarde 1-2 minutos entre buscas.")
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("### 💡 Dicas")
    st.caption("• Aguarde 1-2 min entre buscas")
    st.caption("• Use URLs do formato: /squads/.../")
    ...
    if not times_salvos:
        ...
        if st.button("🔍 Buscar Dados", type="primary", use_container_width=True):
            if not competicao:
                ...
                with st.spinner(f"🔄 Buscando dados de {time_selecionado} na {competicao}... (pode levar até 15 segundos)"):
                ...
                if erro:
                    ...
                    # Área de cópia CSV
                    st.subheader("📋 Dados em Formato CSV (Copiar e Colar)")
                    
                    # Gerar CSV
                    csv_text = df.to_csv(index=False, sep=',')
                    
                    # Exibir em text_area para fácil cópia
                    st.text_area(
                        label="👇 Copie os dados abaixo (Ctrl+A depois Ctrl+C):",
                        value=csv_text,
                        height=300,
                        help="Selecione tudo (Ctrl+A) e copie (Ctrl+C) para usar em Excel, Google Sheets, etc."
                    )
                    
                    st.info("💡 **Dica:** Você pode colar esses dados diretamente no Excel ou Google Sheets!")
                    
                    # Botão de download
                    st.download_button(
                        label="📥 Ou faça Download do CSV",
                        data=csv_text,
...
elif modo == "➕ Adicionar Novo Time":
    st.header("➕ Adicionar Novo Time")
    
    st.info("💡 **Como encontrar a URL:** Vá no FBref, busque o time, copie a URL da página 'Scores & Fixtures' ou 'Stats'")
...
st.markdown(
    ...
    <div style='text-align: center'>
        ...
        <p style='font-size: 12px; color: gray;'>⚠️ Use com moderação - Aguarde entre buscas para evitar bloqueios</p>
    </div>
    """,
